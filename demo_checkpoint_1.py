...
# 0.15 catches small distant targets; ByteTrack filters transient noise.
...
def main():
    ...
    # --- Tracker setup (tuned for drone at altitude) ----------------------
    tracker = sv.ByteTrack(
        ...
        minimum_consecutive_frames=TRACKER_MIN_CONSECUTIVE,
    )
    ...
    while True:
        ...
        try:
            ...
            if isinstance(detections, sv.Detections) and len(detections) > 0:
                ...
                detections.data['class_name'] = np.array(['human'] * len(detections))
